...
impl Executor {
    ...
    fn process_output(output_type: OutputType, redirection_option: Option<Redirection>) {
        let file_path = match &redirection_option {
        Some(Redirection::OutputTruncate(RedirectCode::One(file))) => Some(file),
        Some(Redirection::OutputTruncate(RedirectCode::Two(file))) => Some(file),
        _ => None,
    };
            ...
            match (output_type, redirection) {
                ...
                (output, _) => match output {
                    OutputType::Success(Some(content)) | OutputType::Error(Some(content)) => {
                        //create the redirection file and leave it empty
                        File::create(&file_path.unwrap()).expect("Failed to create file");
                        print!("{}", content);
                    },
